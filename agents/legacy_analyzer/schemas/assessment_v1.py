...
class DataField(BaseModel):
    ...
    name: str = Field(description="Name of the data field, e.g. 'WS-CHOICE'.")
    level: str = Field(description="COBOL level number, e.g. '01', '05'.")
    picture: str | None = Field(
        default=None, description="PICTURE clause if declared, e.g. 'X', '9(4)'."
    )
    section: str = Field(
...
class ModernizationObservation(BaseModel):
    ...
    observation: str = Field(description="Detailed observation supported by this file's code.")
    evidence: SourceEvidence = Field(description="Source citation supporting the observation.")
...
class ScopeDeclaration(BaseModel):
    ...
    source_sha256: str = Field(description="SHA256 hex digest of the raw source file analyzed.")
...
class LegacyAssessment(BaseModel):
    ...
    scope: ScopeDeclaration = Field(description="Execution scope and boundary verification.")
    ...
    call_dependencies: list[CallDependency] = Field(
        default_factory=list, description="External programs invoked via CALL statements."
    )
    menu_options: list[MenuOption] = Field(
        default_factory=list, description="User menu options and branches handled in this program."
    )
    ...
    io_operations: list[IOOperation] = Field(
        default_factory=list, description="Input and output statements (ACCEPT, DISPLAY)."
    )
